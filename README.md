...
| 8 | v0.2.0 收尾發布、citation 與公開入口 | 完成；本機驗收與 hosted CI 通過，準備建立 tag 與 GitHub Release |
...
`v0.2.0` 已通過 211 項 pytest、Ruff、`uv lock --check`、四份 notebook freshness、Citation CFF 1.2.0 schema 與 hosted CI 驗證。wheel／sdist 均通過內容 allowlist；五個 console scripts 也都在乾淨 Python 3.11 環境 exit 0。精確檔案數、bytes 與 SHA-256 保存於 [`reports/phase8/`](reports/phase8/)。
...
本次 Phase 5 最終稽核使用以下完整命令，13 項必要條件均為 `pass`，並回報 `ready_for_handoff=true`、`ready_for_publication=true` 與 `phase5_complete=true`：

```powershell
uv run --no-sync python -m tw_med_qlora.cli.phase5_status `
  --adapter artifacts\phase3-step700-adapter `
  --acceptance-manifest reports\phase5\20260722T131736Z-acceptance.json `
  --publication-receipt reports\phase5\20260722T165957Z-publication-receipt.json `
  --require-complete
```

選配 GGUF 不是 Phase 5 完成的必要條件。
...
本次已取得 A100 40GB、Q4_K_M 匯出的明確核准：預估 0.5–1.0 小時、2.65–5.30 CU，含 20% 緩衝的核准上限為 6.36 CU。成功 run `20260723T074001Z` 的 notebook workflow 實測 1,350.68 秒（22.51 分鐘），主 GGUF 為 7.604 GiB，落在預估 7–9 GiB 內；receipt 沒有保存帳務頁的實際 CU，因此不以執行時間反推為實際扣點。[選配 export notebook](notebooks/export_gguf.ipynb)的 repository gate 已開啟，但每次重建後仍固定 `ENABLE_GGUF_EXPORT=False` 且 approval 欄位留白。只有在 A100 runtime 確認資源面板後，才於第一格填入 notebook 顯示的核准碼；其他格不得手動改動。
