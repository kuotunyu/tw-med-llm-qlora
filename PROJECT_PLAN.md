...
| 2026-07-23 | 解鎖選配 A100 GGUF Q4_K_M 匯出 | 使用者核准 0.5–1.0 小時、2.65–5.30 CU，含 20% 緩衝上限 6.36 CU；只輸出至 Drive，不上傳外部，完成後在 Windows RTX 4090 以 Ollama 驗收。repository gate 開啟，但 notebook 每次重建仍將執行旗標與 approval 欄位重設為關閉 |
...
| 選配 GGUF／Ollama | 已完成；run `20260723T074001Z` 驗證 PEFT class、active adapter 與 672 個 LoRA tensors，Drive 保存 8,165,172,704-byte Q4_K_M 主 GGUF與 854,200,608-byte `mmproj`，逐檔 SHA-256 相符且沒有外部上傳。Windows Ollama 0.32.0／RTX 4090 文字 probe 回傳預期 `C`、4.658 秒；雙 GGUF VLM probe 偵測 `completion`／`vision` 與 CLIP projector，於 0.717 秒回傳預期 `RED`，兩者皆為 100% GPU | [export receipt](reports/phase5/20260723T080232Z-gguf-export-receipt.json)／[文字 acceptance](reports/phase5/20260723T081846Z-ollama-acceptance.json)／[VLM acceptance](reports/phase5/20260723T104400Z-ollama-vlm-acceptance.json)／[notebook](notebooks/export_gguf.ipynb)／[文字驗收腳本](scripts/run_ollama_acceptance.ps1)／[VLM 驗收腳本](scripts/run_ollama_vlm_acceptance.ps1) |
...
| 2026-07-23 | 5 選配 | 重新查核 GGUF／Ollama API 並取得成本核准 | Context7 與官方文件確認 `save_pretrained_gguf(..., q4_k_m)`、OOM 時 `maximum_memory_usage=0.5`、chat template／EOS 一致性，以及 Ollama `FROM ./model.gguf`／`ollama create`；使用者核准 A100 上限 6.36 CU、Drive-only 與 Windows Ollama 驗收 | 開啟 repository gate，強化每次執行閘門、內容安全 receipt 與 Ollama 驗證 |
| 2026-07-23 | 5 選配 | 完成 GGUF 解鎖與 Ollama 驗收契約的本機驗證 | repository gate 已開啟，但 notebook 每次重建仍固定 `ENABLE_GGUF_EXPORT=False` 且 approval 留白；A100／BF16／VRAM／本機與 Drive 空間、archive 大小與 SHA-256、唯一 run 目錄、GGUF／Modelfile hash receipt 皆有 gate。Windows 以假 Ollama CLI 完整跑過 receipt 驗證、100% GPU 判定、固定 probe 與 UTF-8 LF receipt；`pytest` 183 passed、Ruff、`uv lock --check`、四份 notebook freshness 全數通過。Linux/Python 3.12 依賴解析 371 行，固定 `unsloth 2026.7.4`、`torch 2.10.0`、`transformers 4.56.2`、`peft 0.19.1` | 停在付費執行閘門；由使用者建立聚焦 commit 後，在 A100 runtime 只編輯第一格並逐格執行 |
| 2026-07-23 | 5 選配 | 診斷首次 GGUF A100 載入失敗 | run `20260723T033750Z` 通過 A100 40GB、BF16、本機／Drive 空間、Phase 3 archive SHA-256 與 11 個 adapter 檔案 gate；Unsloth 直接載入固定 Hub ID 時，在部分 cache 的離線重試路徑得到 `checkpoint_files=[None]`，於模型載入前停止，未產生 GGUF、Modelfile 或外部上傳 | 停止 A100 runtime；將訓練流程已驗證的完整 snapshot 下載與 shard 稽核移植到 export notebook |
...
| 2026-07-23 | 5 選配 | 完成雙 GGUF VLM 視覺驗收 | 依現行 Ollama API 以兩個 `FROM` 匯入主 GGUF 與 BF16 projector；`/api/show` 回報 `completion`、`vision`、CLIP projector 與兩筆來源。RTX 4090 顯示 100% GPU，合成紅色方塊 probe 於 warm run 0.717 秒回傳預期 `RED`。receipt 綁定 export 與文字 acceptance 的 SHA-256，不保存 raw output、測試圖片、Modelfile 或本機路徑，亦無外部上傳 | 歸檔內容安全 VLM receipt、完成全套測試與本機大型產物清理 |
| 2026-07-23 | 5 選配 | 歸檔 VLM 證據並清理本機大型產物 | 新增 VLM 腳本契約與內容安全 receipt；證據鏈聚焦測試 3 passed，完整 `pytest` 187 passed、Ruff、`uv lock --check` 與四份 notebook freshness 全部通過。精準移除 `tw-med-taide-12b-q4-k-m` 與 `tw-med-taide-12b-q4-k-m-vlm`，其餘 14 個 Ollama 模型逐名核對後不變；刪除本機 run 目錄 8 個檔案共 9,019,384,339 bytes，Drive 備份與三份 tracked receipt 保留 | 由使用者檢視變更、建立聚焦 commits 並 push；確認 hosted Windows／Ubuntu CI |
| 2026-07-23 | 5 選配 | 完成公開快照收尾稽核並收緊 VLM 契約 | 將 VLM gate 收緊為同時要求 `completion`／`vision` 且答案必須精確等於 `RED`，API 位址只允許 loopback；新增 Windows 假 CLI + 假 `/api/show`／`/api/chat` 端到端與非 loopback 拒絕測試，涵蓋雙 `FROM`、base64 圖片、CLIP、100% GPU、LF receipt 與暫存清理。archived evidence 另驗 output hash、正耗時與三個內容排除旗標。新增 6 項公開 snapshot 回歸測試，持續檢查 README 必要章節、本機 Markdown 連結、token 形狀、33 份 report JSON／私密欄位、模型權重／50 MB 大檔與 ignore 規則。workflow checklist 改用實際存在的 `tests/test_project_skill.py`。唯讀遠端複核確認 private adapter HEAD 仍為已驗證 revision `c2830d37…0aa4`，11 個 receipt 檔仍存在。最終 `pytest` 195 passed、Ruff、`uv lock --check`、四份 notebook freshness、project skill 3 tests 與三份 PowerShell parser 全數通過；變更分為 commits `094384c`、`0583960`、`e48b71a` 推送至 `main`，GitHub Actions [run #12](https://github.com/kuotunyu/tw-med-llm-qlora/actions/runs/30010227013) 對 commit `e48b71a88b86014a599e655c7c94014e49b6390d` 的 Windows／Ubuntu jobs 均成功 | 選配 GGUF／Ollama 與公開快照收尾完成 |
...
- 選配 GGUF 匯出已於 A100 完成，Windows Ollama 的文字與 VLM 視覺 probe 均在 100% GPU 載入下通過；repository gate 仍開啟，但每次 notebook 的執行旗標與 approval 欄位預設關閉。receipt 未保存帳務頁實際 CU，因此只報告 22.51 分鐘 workflow 耗時，不把名目 CU／小時換算成實際扣點。VLM 驗收只證明本次雙 GGUF 可辨識合成紅色方塊，不代表一般影像理解、臨床影像判讀或醫療安全性。本機 GGUF 已在內容安全證據歸檔後刪除，若需重驗必須從 Drive 取回同一份 SHA-256 綁定產物。
...
選配 GGUF／Ollama 不改變 Phase 5 已完成的結論，且現已完成。A100 run `20260723T074001Z` 產生經 PEFT／LoRA 證據與逐檔 SHA-256 綁定的 Q4_K_M 主 GGUF及 VLM `mmproj`，只保存至 Google Drive；Windows RTX 4090／Ollama 0.32.0 的文字與雙 GGUF VLM acceptance 均通過。內容安全證據已歸檔，本機兩個 Ollama 測試模型與可由 Drive 還原的 GGUF 暫存資料夾已移除；VLM 契約、證據測試與公開快照稽核亦已推送，GitHub hosted Windows／Ubuntu CI 均成功。
